...
public class TestDataFactory {
    ...
    private HomeAddressEntity buildOxfordStreetHomeAddress() {
        return HomeAddressEntity.builder().addressLine1("Oxford Street").addressLine2("Green")
                                .country(referenceDataFactory.getCountries().get(CountryEntity.GBR)).county(GREATER_LONDON).postCode("EC3")
    ...
    private WorkAddressEntity buildRegentStreetWorkAddress() {
        return WorkAddressEntity.builder().addressLine1("Regent Street").addressLine2("Green")
                                .country(referenceDataFactory.getCountries().get(CountryEntity.GBR)).county(GREATER_LONDON).postCode("EC4")
                                .state(referenceDataFactory.getStates().get(StateCode.ADDRESS_ENABLED)).town(LONDON).build();
    }
}