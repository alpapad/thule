...
if [[ ${PROVISION_LOCALLY} != "true" ]]; then
  ...
  ssh -o StrictHostKeyChecking=no "${PROVISIONING_HOST_USERID}@${PROVISIONING_HOST}" -tt -C "bash -cl '${PROVISIONING_HOST_TARGET_DIRECTORY}/src/main/kubernetes/bin/provision-environment.sh -l $COMMAND_OPTIONS'"
  exit
fi
