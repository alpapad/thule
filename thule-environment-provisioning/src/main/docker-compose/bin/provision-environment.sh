...
usage () {
    ...
    echo "  -s, --service                        Name of service to provision, defaults to all services"
    echo "  -p, --stop                           Stop services once provisioning has completed (typically used for continuous integration testing environments, e.g. TeamCity)"
    echo ""
    echo ""
    exit
}
...
COMMAND_OPTIONS=$@
getoptResults=$(getopt -s bash -o hlps: --long help,provision-locally,service:,stop -- "$@")
...
if [[ ${PROVISION_LOCALLY} != "true" ]]; then
    ...
    ssh -o StrictHostKeyChecking=no ${PROVISIONING_HOST_USERID}@${PROVISIONING_HOST} -C "${PROVISIONING_HOST_TARGET_DIRECTORY}/src/main/bin/docker-compose/provision-environment.sh -l $COMMAND_OPTIONS"
...
STOP_SERVICES is $STOP_SERVICES"
