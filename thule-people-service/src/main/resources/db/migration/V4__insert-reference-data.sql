...
INSERT INTO people(date_of_birth, date_of_expiry, date_of_password_expiry, email_address, first_name, last_name, password, second_name, state_id, title, user_id, version, created_at, created_by, updated_at, updated_by)
    SELECT CURRENT_DATE, DATE_FORMAT('2030-12-31', '%Y-%m-%d'), DATE_FORMAT('2030-12-31', '%Y-%m-%d'), 'superuser@serin-consultancy.co.uk', 'Super', 'User', 'superuser', '', states.id, 'Mr', 'superuser', 1, CURRENT_TIMESTAMP, 'superuser', CURRENT_TIMESTAMP , 'superuser'FROM states WHERE states.code = 'PERSON_ENABLED';
